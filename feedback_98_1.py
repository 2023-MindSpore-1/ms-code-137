...
# np.save("sim.npy",sim)
# sim = np.load("sim.npy")
sim = sio.loadmat('./dataset/cuhk03labeled/ranklist/test/test.mat')
...
for i in range(iteration):
    ...
    for q in range(querynum):
        ...
        for j in range(rankernum):
            ...
            A = []
            for k in range(scored_G.shape[1]):
                x = ms_argwhere(ranker_RT == scored_G[k])
                score = math.ceil(x.asnumpy() / topK)
                new_weight[q, j] = new_weight[q, j] + 1 / (score + 1e-6)

        # total_weight = np.max(new_weight[q,:])
        total_weight = ops_max(new_weight[q, :])
        new_weight[q, :] = new_weight[q, :] / total_weight

    weight = weight * lamda + new_weight * (1 - lamda)
    for j in range(querynum):
        # weight[j,:] = weight[j,:] / np.max(weight[j,:])
        weight[j, :] = weight[j, :] / ops_max(weight[j, :])

    new_sim = ms.Tensor(np.zeros((querynum, gallerynum)), ms.float32)
    for j in range(querynum):
        for k in range(rankernum):
            # new_sim[j,:] = new_sim[j,:] + np.reshape(sim[k,j,:] * weight[j,k], (1,gallerynum))
